...
const Worksheets = () => {
  ...
  const generateCustomWorksheet = () => {
    ...
    const savedWorksheet = saveWorksheet({
      ...
      dicomSeriesConfig: dicomSeriesConfig, // Include DICOM configuration
      machineId: selectedMachineData.machineId // Include machine ID for machine-specific config
    });
  ...
  const saveWorksheet = (worksheetData) => {
    ...
    const newWorksheet = {
      ...
      frequency: worksheetData.frequency,
      tests: worksheetData.tests,
      dicomSeriesConfig: worksheetData.dicomSeriesConfig || dicomSeriesConfig || [],
  ...
  const createWorksheetFromTemplate = (template) => {
    ...
    if (template.dicomSeriesConfig && template.dicomSeriesConfig.length > 0) {
      setDicomSeriesConfig(template.dicomSeriesConfig);
      console.log('DEBUG: Loaded DICOM series configuration:', template.dicomSeriesConfig);
    } else {
      setDicomSeriesConfig([]);
    }
  ...
  return (
    <div className="max-w-6xl mx-auto">
            ...
            <div className="bg-gray-800 rounded-lg p-6">
              ...
              <div className="bg-gray-800 rounded-lg p-6 mb-6">
                <div className="flex items-center justify-between mb-4">
                  ...
                  <div className="text-sm text-gray-400">
                    Configure automated image selection
                  </div>
                </div>
                
                {/* DICOM Configuration Status */}
                <div className="mb-4">
                  {dicomSeriesConfig.length > 0 ? (
                    <div className="bg-green-900/20 border border-green-600 rounded-lg p-4">
                      <div className="flex items-center justify-between">
                        <div>
                          <h3 className="text-green-300 font-medium flex items-center">
                            ✅ DICOM Configuration Complete
                          </h3>
                          <p className="text-green-200 text-sm mt-1">
                            {dicomSeriesConfig.length} DICOM series configured for automatic image selection
                          </p>
                        </div>
                        <button
                          onClick={() => setDicomSeriesConfig([])}
                      ...
                      {/* Show configured series summary */}
                      <div className="mt-3 space-y-2">
                        {dicomSeriesConfig.map((config, index) => (
                          <div key={config.id} className="bg-green-800/30 rounded p-2">
                            <div className="flex items-center justify-between">
                              <span className="text-green-100 font-medium">{config.name || `Series ${index + 1}`}</span>
                              <span className="text-green-300 text-xs px-2 py-1 bg-green-900/50 rounded">
                                {config.priority}
                              </span>
                            </div>
                            <p className="text-green-200 text-sm mt-1">{config.description}</p>
                            <div className="text-xs text-green-300 mt-1">
                              Enables: {config.requiredFor?.join(', ') || 'General QC'}
                            </div>
                          </div>
                        ))}
                      </div>
                    ...
                    <div className="bg-blue-900/20 border border-blue-600 rounded-lg p-4">
                      <div className="flex items-center text-blue-300 text-sm">
                        ...
                        <span>
                          Configure DICOM series identification criteria to enable automatic image selection during QC performance.
                          This is optional - templates can work without DICOM integration.
                        </span>
                      </div>
                    </div>
                  )}
                ...
                {/* Add DICOM Configuration Button */}
                <div className="text-center">
                  ...
                  >
                    <span>🔧</span>
                    <span>{dicomSeriesConfig.length > 0 ? 'Edit DICOM Configuration' : 'Add DICOM Configuration'}</span>
                  </button>
                </div>
              </div>
      ...
      {viewMode === 'dicom-config' && (
        <div className="bg-gray-800 rounded-lg p-6">
          <div className="flex justify-between items-center mb-6">
            <div>
              <h3 className="text-lg font-semibold text-gray-100">DICOM Series Configuration</h3>
              <p className="text-gray-400 text-sm mt-1">
                Configure DICOM series identification criteria for {customWorksheetInfo.modality} {customWorksheetInfo.frequency} QC
              </p>
            </div>
            <button
              onClick={() => setViewMode('templates')}
          ...
          <div className="bg-blue-900/20 border border-blue-600 rounded-lg p-4 mb-6">
            <div className="flex items-center text-blue-300 text-sm">
              ...
              <span>
                ...
                appropriate images when this template is used for QC performance. These settings will be saved with the template.
              </span>
            </div>
          </div>
          ...
          <div className="flex justify-center space-x-4 mt-6">
            <button
              onClick={() => {
                // Save DICOM configuration based on context
                if (worksheetData && !worksheetData.isTemplate) {
                  // We're editing a worksheet's DICOM config - save it machine-specifically
                  const machineId = customWorksheetInfo.machineId || worksheetData.machineId || 
                                   (worksheetData.assignedMachines && worksheetData.assignedMachines[0]);
                  
                  if (machineId) {
                    // Save machine-specific DICOM configuration
                    const saved = saveMachineSpecificDicomConfig(
                      machineId, 
                      worksheetData.modality, 
                      worksheetData.frequency, 
                      dicomSeriesConfig
                    );
                    
                    if (saved) {
                      toast.success(`DICOM configuration saved for ${machines.find(m => m.machineId === machineId)?.name || 'machine'}`);
                      setViewMode('view-only');
                    } else {
                      toast.error('Failed to save DICOM configuration');
                    }
                  } else {
                    toast.error('Machine ID not found for DICOM configuration');
                  }
                  ...
                  toast.success('DICOM configuration updated for template');
                  setViewMode('templates');
                } else {
                  // We're creating a new worksheet, just go back to custom mode
                  setViewMode('custom');
                }
              }}
        ...
        <div className="bg-gray-800 rounded-lg p-6">
          ...
          {!worksheetData.viewOnly && (
            <div className="flex space-x-4 mb-4">
              ...
              <button
                onClick={() => {
                  // Load machine-specific DICOM config for editing
                  const machineId = worksheetData.machineId || (worksheetData.assignedMachines && worksheetData.assignedMachines[0]) || '';
                  
                  // Try to load machine-specific configuration first, then fallback to worksheet config
                  let dicomConfig = [];
                  if (machineId) {
                    dicomConfig = getMachineSpecificDicomConfig(machineId, worksheetData.modality, worksheetData.frequency);
                  }
                  
                  // If no machine-specific config found, try worksheet config, then template config
          ...
          {worksheetData && worksheetData.modality && (
            <DICOMSeriesSelector
              ...
              templateData={worksheetData.viewOnly ? (() => {
                // Load machine-specific DICOM config for view-only mode
                const machineId = worksheetData.machineId || (worksheetData.assignedMachines && worksheetData.assignedMachines[0]);
                let dicomConfig = [];
                ...
                // Fallback to worksheet config if no machine-specific config
                if (dicomConfig.length === 0) {
                  dicomConfig = worksheetData.dicomSeriesConfig || [];
                }
