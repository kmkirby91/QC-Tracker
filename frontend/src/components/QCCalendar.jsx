...
const QCCalendar = ({ qcHistory, type, showOverview = false }) => {
  ...
  useEffect(() => {
    if (worksheetAssignments.length > 0) {
      generateDueDates();
      // Set the first active frequency as default tab
  ...
  }, [worksheetAssignments, qcCompletions]);
  ...
  const getQCStatusForDate = (date, frequency) => {
    ...
    // Check if this date should have QC
  ...
  const renderWeeklyCalendar = () => {
    ...
    const renderDay = (day) => {
      const date = new Date(currentDate.getFullYear(), currentDate.getMonth(), day);
      const qcStatus = getQCStatusForDate(date, 'weekly');
      const isCurrentDay = isToday(date);
      const isWeekendDay = isWeekend(date);
      ...
      return (
        <div
          key={day}
          onClick={() => handleDayClick(day, 'weekly')}
          className={`
            relative h-12 border border-gray-700 flex items-center justify-center text-sm
            ${isCurrentDay ? 'ring-2 ring-blue-500' : ''}
            ${isWeekendDay ? 'bg-gray-900' : 'bg-gray-800'}
  ...
  const renderQuarterlyCalendar = () => {
    ...
    const renderQuarter = (quarter) => {
      const date = new Date(selectedYear, quarter * 3, 1);
      const qcStatus = getQCStatusForDate(date, 'quarterly');
      ...
      return (
        <div
          key={quarter}
          className={`
            relative h-32 border-2 rounded-lg flex flex-col items-center justify-center cursor-pointer
            transition-all duration-200 hover:shadow-md
            ${qcStatus ? getStatusColor(qcStatus.status) : 'bg-gray-900 border-gray-600'}
            ${isCurrentQuarter ? 'ring-2 ring-blue-500' : ''}
          `}
          onClick={() => qcStatus && qcStatus.isOverdue && handleDayClick(1, 'quarterly')}
        >
          <span className={`text-lg font-medium ${qcStatus ? 'text-white' : 'text-gray-400'} mb-2`}>
            {quarterNames[quarter]}
          </span>
          <div className={`text-xs ${qcStatus ? 'text-white' : 'text-gray-400'} text-center`}>
  ...
  const renderAnnualCalendar = () => {
    ...
    const renderYear = (year) => {
      ...
      const qcStatus = getQCStatusForDate(date, 'annual');
      ...
      return (
        <div
          ...
          `}
          onClick={() => qcStatus && qcStatus.isOverdue && handleDayClick(1, 'annual')}
        >
          <span className={`text-2xl font-bold ${qcStatus ? 'text-white' : 'text-gray-400'}`}>
            {year}
          </span>
